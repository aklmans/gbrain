...
function instantiateEmbedding(recipe: Recipe, modelId: string, cfg: AIGatewayConfig): any {
  switch (recipe.implementation) {
    case 'native-openai': {
      const keyName = recipe.auth_env?.required[0] ?? 'OPENAI_API_KEY';
      const apiKey = cfg.env[keyName];
      if (!apiKey) throw new AIConfigError(
        `OpenAI embedding requires ${keyName}.`,
        recipe.setup_hint,
      );
      const baseURL = cfg.base_urls?.[recipe.id] ?? recipe.base_url_default;
      const client = createOpenAI({ apiKey, ...(baseURL ? { baseURL } : {}) } as any);
    ...
    case 'native-google': {
      const keyName = recipe.auth_env?.required[0] ?? 'GOOGLE_GENERATIVE_AI_API_KEY';
      const apiKey = cfg.env[keyName];
      if (!apiKey) throw new AIConfigError(
        `Google embedding requires ${keyName}.`,
        recipe.setup_hint,
      );
      const baseURL = cfg.base_urls?.[recipe.id] ?? recipe.base_url_default;
      const client = createGoogleGenerativeAI({ apiKey, ...(baseURL ? { baseURL } : {}) } as any);
      return (client as any).textEmbeddingModel
        ? (client as any).textEmbeddingModel(modelId)
        : (client as any).embedding(modelId);
    }
...
function instantiateExpansion(recipe: Recipe, modelId: string, cfg: AIGatewayConfig): any {
  switch (recipe.implementation) {
    case 'native-openai': {
      const keyName = recipe.auth_env?.required[0] ?? 'OPENAI_API_KEY';
      const apiKey = cfg.env[keyName];
      if (!apiKey) throw new AIConfigError(`OpenAI expansion requires ${keyName}.`, recipe.setup_hint);
      const baseURL = cfg.base_urls?.[recipe.id] ?? recipe.base_url_default;
      return createOpenAI({ apiKey, ...(baseURL ? { baseURL } : {}) } as any).languageModel(modelId);
    }
    case 'native-google': {
      const keyName = recipe.auth_env?.required[0] ?? 'GOOGLE_GENERATIVE_AI_API_KEY';
      const apiKey = cfg.env[keyName];
      if (!apiKey) throw new AIConfigError(`Google expansion requires ${keyName}.`, recipe.setup_hint);
      const baseURL = cfg.base_urls?.[recipe.id] ?? recipe.base_url_default;
      return createGoogleGenerativeAI({ apiKey, ...(baseURL ? { baseURL } : {}) } as any).languageModel(modelId);
    }
    case 'native-anthropic': {
      const keyName = recipe.auth_env?.required[0] ?? 'ANTHROPIC_API_KEY';
      const apiKey = cfg.env[keyName];
      if (!apiKey) throw new AIConfigError(`Anthropic expansion requires ${keyName}.`, recipe.setup_hint);
      const baseURL = cfg.base_urls?.[recipe.id] ?? recipe.base_url_default;
      return createAnthropic({ apiKey, ...(baseURL ? { baseURL } : {}) } as any).languageModel(modelId);
    }
    case 'openai-compatible': {
      const baseUrl = cfg.base_urls?.[recipe.id] ?? recipe.base_url_default;
...
function instantiateChat(recipe: Recipe, modelId: string, cfg: AIGatewayConfig): any {
  switch (recipe.implementation) {
    case 'native-openai': {
      const keyName = recipe.auth_env?.required[0] ?? 'OPENAI_API_KEY';
      const apiKey = cfg.env[keyName];
      if (!apiKey) throw new AIConfigError(`OpenAI chat requires ${keyName}.`, recipe.setup_hint);
      const baseURL = cfg.base_urls?.[recipe.id] ?? recipe.base_url_default;
      return createOpenAI({ apiKey, ...(baseURL ? { baseURL } : {}) } as any).languageModel(modelId);
    }
    case 'native-google': {
      const keyName = recipe.auth_env?.required[0] ?? 'GOOGLE_GENERATIVE_AI_API_KEY';
      const apiKey = cfg.env[keyName];
      if (!apiKey) throw new AIConfigError(`Google chat requires ${keyName}.`, recipe.setup_hint);
      const baseURL = cfg.base_urls?.[recipe.id] ?? recipe.base_url_default;
      return createGoogleGenerativeAI({ apiKey, ...(baseURL ? { baseURL } : {}) } as any).languageModel(modelId);
    }
    case 'native-anthropic': {
      const keyName = recipe.auth_env?.required[0] ?? 'ANTHROPIC_API_KEY';
      const apiKey = cfg.env[keyName];
      if (!apiKey) throw new AIConfigError(`Anthropic chat requires ${keyName}.`, recipe.setup_hint);
      const baseURL = cfg.base_urls?.[recipe.id] ?? recipe.base_url_default;
      return createAnthropic({ apiKey, ...(baseURL ? { baseURL } : {}) } as any).languageModel(modelId);
    }
    case 'openai-compatible': {
